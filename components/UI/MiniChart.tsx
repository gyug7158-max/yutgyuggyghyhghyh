  ...
  const fetchPart = async (targetSymbol: string, isFut: boolean, useBybit: boolean, tf: string, endTime?: number) => {
    try {
      if (useBybit) {
        ...
        const baseUrl = isFut ? 'https://fapi.binance.com/fapi/v1/klines' : 'https://api.binance.com/api/v3/klines';
        let url = `${baseUrl}?symbol=${targetSymbol.toUpperCase()}&interval=${tf}&limit=600`;
      ...
      console.error(`[MiniChart] Fetch failed for ${useBybit ? 'bybit' : 'binance'}/${isFut ? 'futures' : 'spot'}:`, e);
      return null;
    }
  };
