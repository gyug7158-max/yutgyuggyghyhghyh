...
async function startServer() {
  ...
  app.get("/api/klines/:exchange/:market", async (req, res) => {
    ...
    const { symbol, interval, limit, endTime, end } = req.query;
    try {
      if (exchange === 'binance') {
        const baseUrl = market === 'spot' ? 'https://api.binance.com/api/v3/klines' : 'https://fapi.binance.com/fapi/v1/klines';
        let queryParams = `symbol=${symbol}&interval=${interval || '1h'}&limit=${limit || '24'}`;
        if (endTime) queryParams += `&endTime=${endTime}`;
        const response = await axios.get(`${baseUrl}?${queryParams}`);
        res.json(response.data);
      } else {
        const category = market === 'spot' ? 'spot' : 'linear';
        let queryParams = `category=${category}&symbol=${symbol}&interval=${interval || '60'}&limit=${limit || '24'}`;
        if (end) queryParams += `&end=${end}`;
        const response = await axios.get(`https://api.bybit.com/v5/market/kline?${queryParams}`);
        res.json(response.data);
      }
    } catch (error: any) {
      res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
